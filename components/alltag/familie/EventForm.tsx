...
export default function EventForm({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 bg-subtle rounded-xl p-4">
      <div>
        <label className="block text-xs font-medium text-muted mb-1">{t('event_title')}</label>
      ...
      <div>
        <label className="block text-xs font-medium text-muted mb-1">{t('event_date')}</label>
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required
          className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
      </div>
      <div>
        <label className="block text-xs font-medium text-muted mb-1">{t('event_desc')}</label>
